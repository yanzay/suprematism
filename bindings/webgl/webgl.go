...
// Ported to wasm by Alexey Grachov (@yanzay)
// Original gopherjs version https://github.com/gopherjs/webgl
...
func NewContext(canvas js.Value, ca *ContextAttributes) (*Context, error) {
	...
	gl := canvas.Call("getContext", "webgl", js.ValueOf(attrs))
	if gl == js.Null() {
...
func evaluateTags(ctx *Context) {
	t := reflect.TypeOf(*ctx)
	for i := 0; i < t.NumField(); i++ {
		field := t.Field(i)
		tag := field.Tag.Get("js")
		if tag != "" {
			val := ctx.Value.Get(tag)
			if val == js.Undefined() {
				js.Global().Get("console").Call("debug", "undefined val for tag", tag)
				continue
			}
			reflect.ValueOf(ctx).Elem().Field(i).SetInt(int64(val.Int()))
		}
	}
}
